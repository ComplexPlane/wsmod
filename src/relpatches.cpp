...
Tickable patches[] = {
    ...
    {
        .name = "merged_heaps",
        .main_loop_init_func = gameheaps::init_main_loop,
    },
    ...
    {
        .name = "fix_spooky_action",
        .main_loop_init_func = fix_spooky_action::init_main_loop,
    },
};

const u32 PATCH_COUNT = sizeof(patches) / sizeof(patches[0]);
...
Tickable modules[] = {
    ...
    },
    {
        ...
        .stobj_tick_func = custompack::stobj_tick,
    },
};

const u32 MODULE_COUNT = sizeof(modules) / sizeof(modules[0]);
...
TRAMP(event_info_tick_tramp, mkb::event_info_tick, []() {
    event_info_tick_tramp.chain();
...
void init() {
    HOOK_TRAMP(event_info_tick_tramp);
}
}  // namespace perfect_bonus
            ...
            for (u32 col_hdr_idx = 0; col_hdr_idx < (mkb::stagedef->coli_header_count);
                ...
                for (u32 refl_idx = 0; refl_idx < hdr->reflective_stage_model_count; refl_idx++) {
                    ...
                    distance_to_mirror = get_distance(
                        current_ball_position, refl->g_model_header_ptr->bound_sphere_center);
    ...
    if (mkb::sub_mode == mkb::SMD_SEL_NGC_MAIN) {
        ...
        if (mkb::g_currently_visible_menu_screen == 0x6) {
            if (pad::button_pressed(mkb::PAD_BUTTON_A)) {
                ...
                *(&mkb::sel_menu_info.menu_stack_ptr + 2) = 7;
...
// Skips the first cutscene. Replaces the original dmd_scen_newgame_main function, which sets up
// cutscene stuff.
...
// Removes the playpoint notification screens when exiting from story mode or challenge mode, or
// after a 'game over'.
...
// Fixes an issue with rain droplets not appearing correctly on the continue platform in the storm
// theme.
...
// Fixes an issue where skipping more than a certain number of stages for story mode any% causes a
// crash.
...
TRAMP(tex_load_tramp, mkb::g_load_preview_texture,
      [](mkb::SpriteTex* sprite_tex, char* file_path, u32 param_3, u16 width, u16 height,
         u32 format) {
          ...
          tex_load_tramp.chain(sprite_tex, file_path, param_3, width, height, format);
      });

void init_main_loop() {
    HOOK_TRAMP(tex_load_tramp);
}
...
void sel_ngc_init() {
    ...
    patch::write_word(relutil::relocate_addr(0x808f9154),
                      PPC_INSTR_LI(PPC_R0, (~party_game_bitflag & 0x3f)));

    mkb::strcpy(mkb::CANNOT_SELECT_PARTY_GAME_STRING,
                "You cannot play this game\n in this custom pack.");
    mkb::strcpy(mkb::CAN_PURCHASE_PARTY_GAME_STRING,
                "You cannot unlock this game\n in this custom pack.");
    mkb::sprintf(mkb::CAN_PLAY_NUM_PARTY_GAMES_STRING,
...
// TODO: If/when I make a patch/toggle for changing the menu BG stage slots, don't hardcode the
// value.
...
TRAMP(s_load_stage_1_tramp, mkb::queue_stage_load, [](u32 stage_id) {
    rendefc_handler(stage_id);
    s_load_stage_1_tramp.chain(stage_id);
});

TRAMP(s_load_stage_tramp, mkb::load_stage, [](int stage_id) {
    rendefc_handler(stage_id);
    s_load_stage_tramp.chain(stage_id);
});

void init_main_loop() {
    HOOK_TRAMP(s_load_stage_1_tramp);
    HOOK_TRAMP(s_load_stage_tramp);
}
}  // namespace enable_menu_reflections

// Allows for the number of worlds in story mode to be customized.
namespace custom_world_count {
TRAMP(s_sceneplay_init_tramp, mkb::dmd_scen_sceneplay_init, dmd_scen_sceneplay_init_patch);
...
void init_main_game() {
    ...
    if (!patches[Patches::SKIP_CUTSCENES].status) {
        ...
        HOOK_TRAMP(s_sceneplay_init_tramp);
    }
}
...
void dmd_scen_sceneplay_init_patch() {
    ...
    s_sceneplay_init_tramp.chain();
}
}  // namespace custom_world_count

namespace stobj_draw_fix {
/*
 * Stobjs (goaltapes, party balls, bumpers, etc) placed on itemgroups with index greater than 127
 * may get an incorrect itemgroup transform, making them appear weirdly or not at all. This patch
 * forces stobj itemgroup indices to be treated as unsigned bytes instead, increasing the greatest
 * itemgroup idx a stobj may reside on to 255.
 */

namespace {
// These are Ghidra addresses...
const u16 lbz_addrs_lo[] = {
    0x0fb8, 0x0fb8, 0x0fcc, 0x1aa0, 0x1aa0, 0x1abc, 0x1abc, 0x1ce4, 0x1ce4, 0x1d08, 0x1d08, 0x1d34,
    0x1d34, 0x3cf0, 0x4ea4, 0x4ecc, 0x4f64, 0x6500, 0x6570, 0x6a64, 0x7208, 0x7c5c, 0x7c98, 0x7d34,
};
}  // namespace
...
// Fixes an issue where accumulated floating-point error during collision calculations
// may apply a small undesired position/velocity change to the ball. This is most
// noticeable on stages with many, fast, or very rotational animations. Keeps a clean
// copy of the ball's physics state around and restores it if collision with an
// itemgroup did not actually transform the ball.
...
TRAMP(s_tf_physball_tramp, mkb::tf_physball_to_itemgroup_space,
      [](mkb::PhysicsBall* physicsball, int dest_ig_idx) {
          ...
          s_tf_physball_tramp.chain(physicsball, dest_ig_idx);
      });
...
TRAMP(s_collide_physicsball_tramp, mkb::collide_ball_with_plane,
      [](mkb::PhysicsBall* physicsball, mkb::ColiPlane* plane) {
          ...
          mark_coli_flag_if_transformed(prev_pos, prev_vel, physicsball);
      });
...
void init_main_loop() {
    HOOK_TRAMP(s_init_physicsball_tramp);
    HOOK_TRAMP(s_tf_physball_tramp);
    HOOK_TRAMP(s_collide_physicsball_tramp);
    HOOK_TRAMP(s_stobj_bumper_coli_tramp);
    HOOK_TRAMP(s_stobj_goalbag_coli_tramp);
    HOOK_TRAMP(s_stobj_goaltape_coli_tramp);
    HOOK_TRAMP(s_stobj_returngate_coli_tramp);
}
}  // namespace fix_spooky_action
